...
def append_element(index: DatasetIndex, new_element: Union[VolumeSource, DatasetViewCollection]) -> DatasetIndex:
...
def sync_elements(index: DatasetIndex, element_path: str) -> DatasetIndex:
    new_index = index.dict()
    new_index['volumes'] = sorted(list(map(materialize_element, Path(element_path).glob('*.json'))), key = lambda v: v.name)
    return DatasetIndex(**new_index)
...
def main(index_path: str, element_path: str):
    ...
    # appending
        ...
        new_element: Union[VolumeSource, DatasetViewCollection] = materialize_element(element_path)
